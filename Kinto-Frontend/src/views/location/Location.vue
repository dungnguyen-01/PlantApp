        ...
        <CCardBody>
          ...
          <CRow>
            <div class="table-responsive-lg">
              <table class="table">
                ...
                <tbody>
                  <tr
                    v-for="(location, index) in locations.content"
                    :key="index"
                  >
                    <td>{{ index + 1 }}</td>
                    <td>{{ location.code }}</td>
                    ...
                    <td>
                      {{ this.$formatDate.formatDate(location.createAt) }}
                    </td>
                    <td>
                      {{ this.$formatDate.formatDate(location.updateAt) }}
                    </td>
                    <td>
                      <button
                        class="btn btn-outline-info rounded-pill p-0 px-4"
                        title="detail"
                        v-on:click="detailLocation(location.id)"
                      >
                    ...
                    <td>
                      <button
                        ...
                        title="edit"
                        v-on:click="updateLocation(location.id)"
                        v-show="
          ...
          <CRow>
            ...
            >
              ...
              <CPaginationItem
                style="cursor: pointer"
                v-for="(page, index) in locations.totalPages"
                :key="index"
                @click="setPage(page)"
              ...
              <CPaginationItem
                style="cursor: pointer"
                v-show="pageNumber != locations.totalPages"
                v-bind:hidden="locations.totalPages == 0"
                @click="nextPage()"
...
export default {
  ...
  methods: {
    ...
    detailLocation(id) {
      this.$router.push(`/location/detail-location/${id}`)
    },
    addLoaction() {
